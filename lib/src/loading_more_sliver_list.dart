...
class LoadingMoreSliverList<T> extends StatefulWidget {
  ...
  @override
  State<LoadingMoreSliverList<T>> createState() => _LoadingMoreSliverListState<T>();
}

class _LoadingMoreSliverListState<T> extends State<LoadingMoreSliverList<T>> {
  @override
  Widget build(BuildContext context) {
    final LoadingMoreBase<T> sourceList = widget.sliverListConfig.sourceList;
    final bool useProviderValue = widget.sliverListConfig.useProviderValue;
    final Widget child = Consumer<LoadingMoreBase<T>>(
      builder: (BuildContext context, LoadingMoreBase<T> source, Widget? child) {
        return widget.sliverListConfig.buildContent(context, source);
      },
    );

    if (useProviderValue) {
      return ChangeNotifierProvider<LoadingMoreBase<T>>.value(
          value: sourceList, child: child);
    }
    return ChangeNotifierProvider<LoadingMoreBase<T>>(
        create: (_) => sourceList, child: child);
  }
}
