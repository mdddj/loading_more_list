...
class LoadingMoreListConfig<T> {
  ...
  Widget? buildErrorItem(BuildContext context) {
    final bool hasError = sourceList.indicatorStatus
            is IndicatorStatusModelWithError ||
        sourceList.indicatorStatus is IndicatorStatusModelWithFullScreenError;
    if (hasError) {
      Widget? widget;
      if (indicatorBuilder != null)
        widget = indicatorBuilder!(context, sourceList.indicatorStatus);
      widget = widget ??
          IndicatorWidget(
            sourceList.indicatorStatus,
            isSliver: isSliver,
